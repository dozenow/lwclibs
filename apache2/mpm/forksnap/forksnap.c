...
/* data retained by forksnap across load/unload of the module
 * allocated on first call to pre-config hook; located on
 * subsequent calls to pre-config hook
 */
typedef struct forksnap_retained_data {
...
} forksnap_retained_data;
static forksnap_retained_data *retained;

typedef struct forksnap_child_bucket {
    ap_pod_t *pod;
    ap_listen_rec *listeners;
    apr_proc_mutex_t *mutex;
} forksnap_child_bucket;
static int                   num_buckets; /* Number of listeners buckets */
static forksnap_child_bucket *all_buckets, /* All listeners buckets */
...
static void forksnap_note_child_killed(int childnum, pid_t pid,
                                      ap_generation_t gen)
{
...
static void forksnap_note_child_started(int slot, pid_t pid)
{
...
static void clean_child_exit(int code)
{
    ...
    if (one_process) {
        forksnap_note_child_killed(/* slot */ 0, 0, 0);
    }
...
static int forksnap_query(int query_code, int *result, apr_status_t *rv)
{
    *rv = APR_SUCCESS;
    switch(query_code){
...
static const char *forksnap_get_name(void)
{
    return "forksnap";
}
...
static void child_main(int child_num_arg, int child_bucket)
{
    ...
    while (!die_now && !shutdown_pending) {
         ...
         */
        //apr_pool_clear(ptrans); snaptastic
...
static int make_child(server_rec *s, int slot, int bucket)
{
...
#endif
        apr_signal(SIGTERM, sig_term);
        forksnap_note_child_started(slot, getpid());
        child_main(slot, 0);
        /* NOTREACHED */
        ap_assert(0);
        return -1;
    ...
    forksnap_note_child_started(slot, pid);
...
static int forksnap_run(apr_pool_t *_pconf, apr_pool_t *plog, server_rec *s)
{
    int index;
        ...
        if (pid.pid != -1) {
            ...
            if (child_slot >= 0) {
                (void) ap_update_child_status_from_indexes(child_slot, 0, SERVER_DEAD,
                                                           (request_rec *) NULL);
                forksnap_note_child_killed(child_slot, 0, 0);
    ...
    if (shutdown_pending && !retained->is_graceful) {
        ...
        ap_relieve_child_processes(forksnap_note_child_killed);
 ...
 */
static int forksnap_open_logs(apr_pool_t *p, apr_pool_t *plog, apr_pool_t *ptemp, server_rec *s)
{
    int startup = 0;
    int level_flags = 0;
...
static int forksnap_pre_config(apr_pool_t *p, apr_pool_t *plog, apr_pool_t *ptemp)
{
    int no_detach, debug, foreground;
    apr_status_t rv;
    const char *userdata_key = "mpm_forksnap_module";
...
static int forksnap_check_config(apr_pool_t *p, apr_pool_t *plog,
                                apr_pool_t *ptemp, server_rec *s)
{
    ...
    /* ap_daemons_to_start > ap_daemons_limit checked in forksnap_run() */
    ...
    /* ap_daemons_max_free < ap_daemons_min_free + 1 checked in forksnap_run() */

    return OK;
}

static void forksnap_hooks(apr_pool_t *p)
{
    ...
    ap_hook_open_logs(forksnap_open_logs, NULL, aszSucc, APR_HOOK_REALLY_FIRST);
    /* we need to set the MPM state before other pre-config hooks use MPM query
     * to retrieve it, so register as REALLY_FIRST
     */
    ap_hook_pre_config(forksnap_pre_config, NULL, NULL, APR_HOOK_REALLY_FIRST);
    ap_hook_check_config(forksnap_check_config, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_mpm(forksnap_run, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_mpm_query(forksnap_query, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_mpm_get_name(forksnap_get_name, NULL, NULL, APR_HOOK_MIDDLE);
}
...
static const command_rec forksnap_cmds[] = {
LISTEN_COMMANDS,
...
AP_DECLARE_MODULE(mpm_forksnap) = {
    MPM20_MODULE_STUFF,
    ...
    forksnap_cmds,               /* command apr_table_t */
    forksnap_hooks,              /* register hooks */
};
