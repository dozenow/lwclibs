...
/* a += b */
void timeadd(struct timespec *a, struct timespec *b) {
	if (a->tv_nsec + b->tv_nsec > 1000000000) {
		a->tv_sec++; //assumption is it's not over by more than one
...
int readopts(int argc, char *const argv[], struct opts *res) {
	...
	const char usage[] ="options\n"
		...
		"\t-w Number of creations in the time window before outputting timing statistics\n"
	...
	while(!rv && ((c=getopt(argc, argv, optstring)) != -1)) {
		...
		case 's':
			...
			break;
		case 'w':
			rv = getuint(optarg, &res->creations);
			break;
		case 'c':
...
int do_child(int pipe, const struct opts *opts) {
	...
	for(;;) {
		...
		if (creations >= opts->creations) {
			struct record rec = {
				.creations = creations,
				.switches = switches
			};
			clock_gettime(CLOCK_REALTIME, &end);
			timediff(&start, &end, &rec.elapsed);
	...
	for(unsigned w = 0; (g_options.iterations == 0 || w < g_options.iterations) ;++w) {
		...
		for(unsigned int i = 0; i < g_options.children; ++i) {
			if (kids[i].pid == -1) {
				...
				timeadd(&sum.elapsed, &rec.elapsed);
				//printf("parent read child %d\n", i);
			}
		}
		fprintf(g_options.fp, "For window %d got %d creations and %d switches in %ld.%09ld seconds\n", 
		        w, sum.creations, sum.switches, sum.elapsed.tv_sec, sum.elapsed.tv_nsec);
	}
